...
export class AccountManagerDO extends DurableObject<Env> {
  constructor(ctx: DurableObjectState, env: Env) {
    super(ctx, env);
  }

  async register(record: AccountRecord): Promise<string> {
    const accounts = await this.ctx.storage.get<AccountRecord[]>(STORAGE_KEY) || [];
    ...
    await this.ctx.storage.put(STORAGE_KEY, accounts);
